...
merged[, V1 := NULL]
setnames(merged, old = names(merged), new = c("t1","t2","t3","weight.x1"))
setkeyv(merged, c("t1", "t2", "t3"))
pb <- txtProgressBar(min=1, max=length(file_list), style=3)
for(i in 2:length(file_list)){
    file2 <- fread(file_list[i])
    file2[, V1 := NULL]
    merged <- merge(merged, file2, by = c("t1", "t2", "t3"), all = TRUE)
...
write.csv(merged, "./data/merged_table.csv")
close(pb)